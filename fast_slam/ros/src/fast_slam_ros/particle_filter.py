...
class ParticleFilter():
	...
	def particle_publisher(self):
		...
		#for i in new_particle_list:
		#	print(i.w)
		max_weight_particle=new_particle_list[0]
		...
		#------------------------- AQUI ESTA SE A USAR COMO MAPA A PARTICULA DE MAIOR PESO ---------------------------------------
		
		(markers_aux , size , rotation_matrix , translation_matrix )= max_weight_particle.kf.markers_publisher(self.mean_error,self.std,True)
		...
		total_weight=0
		for i in new_particle_list:
			if np.abs(np.sum(np.abs(np.array([i.x,i.y])-np.array([max_weight_particle.x,max_weight_particle.y]))**2,axis=-1)**(1./2)) < 0.5 :
				pose_estimate.pose.position.x+=i.w*i.x
				pose_estimate.pose.position.y+=i.w*i.y
				total_weight+=i.w
		
		pose_estimate.pose.position.z=0.275
		pose_estimate.pose.position.y=float(pose_estimate.pose.position.y)/total_weight
		pose_estimate.pose.position.x=float(pose_estimate.pose.position.x)/total_weight
		procrustes_pose =  np.dot(np.array([pose_estimate.pose.position.x,pose_estimate.pose.position.y]),rotation_matrix)  + translation_matrix;

		pose_estimate.pose.position.x=procrustes_pose[0]
		pose_estimate.pose.position.y=procrustes_pose[1]

		aquele_alfap=max_weight_particle.alfap
		(px,py,pz,pw) = tf.transformations.quaternion_from_euler(0,0,aquele_alfap)
		...
		for i in self.particle_list:
			if i!=None:				
				...
				(markers , size , rotation_matrix , translation_matrix)=i.kf.markers_publisher(self.mean_error,self.std)
				
				procrustes_pose =  np.dot(np.array([i.x,i.y]),rotation_matrix)  + translation_matrix;

				aux_pose=Pose()
				aux_pose.position.x=procrustes_pose[0]
				aux_pose.position.y=procrustes_pose[1]
